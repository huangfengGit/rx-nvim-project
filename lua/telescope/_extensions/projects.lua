...
local function readPrettierConfig(dir)
  if dir ~= nil then
    local file = io.open(dir .. "/.prettierrc", "r")
    if file then
      for line in file:lines() do
        local start_pos, end_pos = string.find(line, "tabWidth")
        if start_pos then
          local len = string.len(line)
          local second_to_last_char = string.sub(line, len - 1, len - 1)
          vim.notify("tab width:" .. second_to_last_char)
          vim.opt.shiftwidth = tonumber(second_to_last_char)
          vim.opt.tabstop = tonumber(second_to_last_char)
        end
      end
      file:close()
    else
...
local function change_working_directory(prompt_bufnr, prompt)
  ...
  readPrettierConfig(project_path)
